...
const SquareFormat = ({pageData, workSpaceSlice, pageDataSelected, isInWorkSpcae, loading}) => {
	...
	if (loading) {
		return (
			<Skeleton className="SquareFormat isSinglePage" />
		);
	}
	return (
		<div className={`SquareFormat ${(!pageData?.sheet2 && !isSinglePage) && "isOnePage"} ${isSinglePage && "isSinglePage"}`}>
			<div
				className={
...
export default connect(mapStateToProps, mapDispatchToProps) (SquareFormat);
